...
keyboard1.row('Показать вопросы')
questions = {
        1:[
...
def send_text(message):
    ...
    if message.text == 'Показать вопросы':
        ...
        button = []
        for count in range(1, len(questions)+1):
            button.append(types.InlineKeyboardButton(text=questions[count][0], callback_data=count))
        keyboard.add(*button)
...
def callback_worker(call):
    global questions
    #bot.send_message(call.from_user.id, text=questions[int(call.data)][0])
    bot.send_message(call.message.chat.id, questions[int(call.data)][1])
